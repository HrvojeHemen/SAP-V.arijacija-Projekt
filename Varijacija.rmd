...
Vidimo da svi filmovi imaju zapisane žanrove i ocjene tako da možemo krenuti u analizu podataka.
...
```
Vidimo kako samo žanr "Biography" značajnije odstupa od drugih standardnih varijacija.
Ovo nas navodi na činjenicu da ne postoji značajna razlika u ocjenama između pojedinih žanrova.
...
#Izdvajamo samo stupce sa imenima filmova i ocjenama
biography <- biography[c(12,26)]
horror <- horror[c(12,26)]
...
cor(cbind(gross,num_voted_users,movie_facebook_likes,cast_total_facebook_likes,
          imdb_score))
```
Iz matrice vidimo da zarada nema vezu niti s jednom varijablom.


## Možemo li što zaključiti iz podataka o Facebook lajkovima glavnih glumaca, redatelja te samog filma?

Zanima nas možemo li nešto zaključiti iz podataka o Facebook lajkovima vezanih uz pojedine filmove.

Prvo pogledajmo s kojim sve varijablama raspolažemo.
```{r}
names(imdb)
```
Naše područje interesa primarno obuhvaća varijable: "director_facebook_likes", "actor_1_facebook_likes", "actor_2_facebook_likes", "actor_3_facebook_likes", "movie_facebook_likes". Također, po potrebi im možemo priključiti i varijablu "cast_total_facebook_likes".

Pogledajmo kako se ponašaju te varijable.
```{r}

imdb_facebook_likes = select(imdb, c("director_facebook_likes", "actor_1_facebook_likes", "actor_2_facebook_likes", "actor_3_facebook_likes", "cast_total_facebook_likes", "movie_facebook_likes"))

summary(imdb_facebook_likes)

```
Podatci su naravno "integer" vrste.

Odmah se može uočiti da su rangovi promatranih varijabli veliki, te da se srednje vrijednosti i medijani značajno razlikuju. Kod svih šest varijabli, srednja vrijednost je višestruko veća od medijana, što znači da je riječ o zakrivljenim distribucija, točnije pozitivno (ili desno) zakrivljenim distribucijama.

Zaključak da je riječ o zakrivljenim distribucijama, možemo poduprijeti i grafički:
```{r}

hist(imdb$director_facebook_likes,main='Director Facebook likes', xlab='Likes', ylab='Frequency')
hist(imdb$actor_1_facebook_likes,main='Actor1 Facebook likes', xlab='Likes', ylab='Frequency')
hist(imdb$actor_2_facebook_likes,main='Actor2 Facebook likes', xlab='Likes', ylab='Frequency')
hist(imdb$actor_3_facebook_likes,main='Actor3 Facebook likes', xlab='Likes', ylab='Frequency')
hist(imdb$cast_total_facebook_likes,main='Cast Total Facebook likes', xlab='Likes', ylab='Frequency')
hist(imdb$movie_facebook_likes,main='Movie Facebook likes', xlab='Likes', ylab='Frequency')

```
Radi bolje vizualizacije podataka, možemo pokušati približiti podatke normalnoj distribuciji korištenjem log transformacije:
```{r}

hist(log(imdb$director_facebook_likes),main='Director Facebook likes', xlab='Likes', ylab='Frequency')
hist(log(imdb$actor_1_facebook_likes),main='Actor1 Facebook likes', xlab='Likes', ylab='Frequency')
hist(log(imdb$actor_2_facebook_likes),main='Actor2 Facebook likes', xlab='Likes', ylab='Frequency')
hist(log(imdb$actor_3_facebook_likes),main='Actor3 Facebook likes', xlab='Likes', ylab='Frequency')
hist(log(imdb$cast_total_facebook_likes),main='Cast Total Facebook likes', xlab='Likes', ylab='Frequency')
hist(log(imdb$movie_facebook_likes),main='Movie Facebook likes', xlab='Likes', ylab='Frequency')

```

Nadalje, možemo uočiti da se kod svih promatranih varijabli donjih 75% podataka nalazi unutar višestruko manjeg intervala, od onog intervala unutar kojeg se nalazi gornjih 25% podataka, npr. 75% redatelja za svoje filmove dobilo je između 0 i 195 lajkova, dok je 25% redatelja dobilo između 195 i 23 000 lajkova. Naravno, ta je činjenica u skladu s činjenicom da je riječ o pozitivno zakrivljenim distribucijama.

U konačnici, pogledajmo još koliki točno postotak sačinjavaju nedostajuće (NA) vrijednosti.
```{r}

for (varijabla in names(imdb_facebook_likes)){
  if (sum(is.na(imdb_facebook_likes[,varijabla])) > 0){
    cat('Postotak nedostajućih (NA) vrijednosti za varijablu ', varijabla, ': ', (sum(is.na(imdb_facebook_likes[,varijabla])) / nrow(imdb_facebook_likes)) * 100, '%\n')
  } else {
    cat('Postotak nedostajućih (NA) vrijednosti za varijablu ', varijabla, ': 0\n')
  }
}

```
Vidimo da su to mali postotci pa nam neće biti problem ukloniti ih za potrebe analize.